...
/**
 * Instruction BEQ — saut conditionnel si deux registres sont égaux.
 *
 * Exemple assembleur : BEQ r0, r1, @50
 * Ce que ça fait     : SI r0 == r1 ALORS PC = 50
 *                      SINON continue normalement
 *
 * Octets en mémoire  : [12][00][01][00][32]
 *                        ↑   ↑   ↑   ↑   ↑
...
public class BeqInstruction extends Instruction {
    ...
    /**
     * Construit une instruction BEQ.
     *
     * @param regA    numéro du premier registre (0 à 15)
     * @param regB    numéro du deuxième registre (0 à 15)
     * @param address adresse de destination si égaux (0 à 65535)
     * @param pc      le compteur de programme du CPU
     */
    public BeqInstruction(int regA, int regB, int address, ProgramCounter pc) {
        super(Opcode.BEQ);
        this.regA    = regA;
        this.regB    = regB;
    ...
    /**
     * Exécute l'instruction BEQ.
     *
     * Étapes :
     * 1. Lit les valeurs de regA et regB
     * 2. Compare les deux valeurs
     * 3. Si égales → modifie le PC pour sauter à l'adresse
    ...
    public void execute(RegisterFile registerFile, Memory memory, ALU alu)
        ...
        // 2. Compare les deux valeurs
        // Si égales → saute à l'adresse destination
        if (valeurA == valeurB) {
            pc.set(address);
        }
        // Sinon → ne fait rien, le CPU continue normalement
